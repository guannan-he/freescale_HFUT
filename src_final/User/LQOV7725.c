...
*            如果面积为正数，数值越大说明越偏右边。                        *
***************************************************************************/ 
void Seek_Road(int* center_line)
{  
  int nr; //行
  int nc; //列
  int temp_l=0;//临时数值
...
采集一帧以后就返回主函数
***********************************************************************/
void camera(int* road_data,int* center_line)
{
  char txt[8];
...
*/
  ...
  //if(Field_Over_Flag)    //完成一场图像采集
    //{                                   
      ...
      //Pixle_Filter();
      if_start();//启停检测
      Seek_Road(center_line);//返回位置
      //improved_seek_road();//废弃，无限复位
      course_detect(center_line);//中线绘制
      Draw_Road();//画路
      if(start_stop==1)
      {
        LED_Ctrl(LED1,ON);
...
能改的则改，不能改的坚决不改*/
void course_detect (int* center_line)
{
  int i=0;
  for(i=0;i<LCDH;i++)
  {
