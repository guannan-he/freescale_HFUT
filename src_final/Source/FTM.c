...
void FTM_AB_clear(FTM_Type * ftmn)
{
  FTM_CNT_REG(ftmn) = 0;
}
...
void Motor_Ctrl(u16 duty)
{
  ...
  FTM_CnV_REG(FTM0, FTM_CH1) = 1100;
  FTM_CnV_REG(FTM0, FTM_CH0) = duty;
}
void Motor_Ctrl_reverse(u16 duty)
{
  if (duty>1100)
  {
   duty=1100;
...
void Motor_Init(int Step_Middle)
{
  //舵机频率：bus_clk/(2^6)/?=50HZ,    
  //电机频率：bus_clk/(2^3)/?=12.5kHZ,  比如110M/12500/8=1100
  FTM_PWM_Init(FTM0,FTM_CH0,bus_clk_M*1000000/12500/8,1100);//Mot0-PTC1
  FTM_PWM_Init(FTM0,FTM_CH1,bus_clk_M*1000000/12500/8,1100);//Mot1-PTC2 
  FTM_PWM_Init(FTM0,FTM_CH2,bus_clk_M*1000000/12500/8,1100);//Mot2-PTC3
  FTM_PWM_Init(FTM0,FTM_CH3,bus_clk_M*1000000/12500/8,1100);//Mot3-PTC4
  Servo_FTM_PWM_Init(FTM3,FTM_CH6,bus_clk_M*1000000/64/50,Step_Middle);//Mot11-PTC10    舵机方向左边大右边小
  Servo_FTM_PWM_Init(FTM3,FTM_CH7,bus_clk_M*1000000/64/50,Step_Middle);//Mot11-PTC11    舵机方向左边大右边小
}

void Step_Angle_Set(s16 duty,int Step_Left,int Step_Right)
{//舵机打角度在一定范围内，不可超越，否则卡死后舵机容易烧掉           
  if(duty<Step_Left) duty=Step_Left;
  else if(duty>Step_Right) duty=Step_Right;
  FTM_CnV_REG(FTM3, FTM_CH6) = duty; 
  FTM_CnV_REG(FTM3, FTM_CH7) = duty; 
}
...
void updat_pid(PIDP* pid)
{
  ...
  pid->istate += ((pid->iGain)*(pid->errdat)*0.02);//integrater
  if((pid->istate)>(pid->ilimit))
    pid->istate = pid->ilimit;
  if((pid->istate)<(ilimit1))
    pid->istate = ilimit1;
  pid->pidout = (pid->pGain) * ((pid->errdat) + (pid->istate) + (((pid->errdat)-(pid->perr)) * (pid->dGain))*50);
  pid->perr=pid->errdat;
}
